...
class ShellScript:
    ...
    def generate_script(self):
        ...
        if self.keepfiles.keep_files_common_path:
            self.script.append(
                f"# There's a common path for the selected files: {self.keepfiles.keep_files_common_path}"
            )
            self.script.append(
                "# As an optimization we'll run subdirectory-filter on the common path first"
            )
            self.script.append(
                f"git filter-branch --force --prune-empty --subdirectory-filter {self.keepfiles.keep_files_common_path}"
            )
            self.script.append("")
        ...
        self.script.append(
            f'git filter-branch --force --prune-empty --tree-filter "{keepfiles_cmds}"'
        )
        self.script.append("")
        self.script.append(
